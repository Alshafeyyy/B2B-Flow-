...
def parse_args():
    ...
    parser.add_argument("--deep-dive", type=str, default=None, metavar="COMPANY", help="Skip company search entirely and deep-research ONE known company by name, e.g. --deep-dive \"Sothema\". Produces a company dossier + deep individual research per selected contact (professional background, public presence, meeting prep). Ignores --locations/--keywords/--industries/--limit etc. CAUTION: a loose/abbreviated name can silently match the wrong company (e.g. \"PMI\" matches both Project Management Institute and Philip Morris International) — this CLI mode trusts the first match with no confirmation step, so prefer the exact name or the website domain (e.g. \"sothema.ma\") for reliability. The web app's Deep-Dive mode shows candidates to confirm before running.")
